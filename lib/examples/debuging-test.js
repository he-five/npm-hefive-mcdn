...
function callbackAfterSonSof(data) {
    ...
    driver.sendCmd(index_1.Commands.STATUS, testCallbackStatus);
}
function testCallbackStatus(data) {
    console.log(`testCallbackStatus: ${JSON.stringify(data)}`);
    driver.sendCmdDataNumber(index_1.CommandsData.RelativeMove, new robot_cmd_1.RobotData('T', 1000), testCallbackAfetrMvr);
    //driver.disconnect()
}
...
driver.on('connected', (data) => {
    ...
    driver.sendCmd(index_1.Commands.FW_VER, testCallback2);
    //driver.sendCmd(Commands.SERVO_ON, callbackAfterSonSof)
    driver.sendCmd(index_1.Commands.SERVO_ON, callbackAfterSonSof);
    //  setTimeout(() => {
