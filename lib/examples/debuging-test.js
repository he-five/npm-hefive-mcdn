...
driver.on('connected', (data) => {
    ...
    driver.sendCmd(commands_1.Commands.STATUS, testCallback2);
    setTimeout(() => {
        driver.sendCmd(commands_1.Commands.SERVO_OFF);
        //driver.sendCmd(Commands.POWER_OFF);
        driver.sendCmd(commands_1.Commands.STATUS, testCallback1);
        setTimeout(() => {
            ...
            driver.sendCmd(commands_1.Commands.INPUTS);
            //driver.disconnect()
        }, 1000);
        //driver.disconnect()
    }, 1000);
...
driver.on('data', (data) => {
    ...
    if (data.cmd === commands_1.Commands.POWER_ON) {
        //driver.sendCmd(Commands.RelativeMove, new RelativeMove(100));
        //console.timeEnd('EXECUTION TIME \'ver\'')
    }
    if (data.cmd === commands_1.Commands.INPUTS) {
        setTimeout(() => {
            //     driver.sendCmd(Commands.STATUS, testCallback)
            // driver.sendCmd(Commands.SERVO_ON);
            // driver.sendCmd(Commands.POWER_ON);
