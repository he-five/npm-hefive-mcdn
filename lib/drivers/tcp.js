    ...
    sendThruPort(cmd) {
        ...
        switch (this.cmd) {
            ...
            case commands_1.Commands.SERVO_OFF:
                actualCmd = '.noservo';
                break;
            case commands_data_1.CommandsData.SyncRelMove:
            case commands_data_1.CommandsData.SyncAbsMove:
                if (cmd.data) {
                    let data = cmd.data;
                    if (data) {
                        let moveCmd = this.cmd === commands_data_1.CommandsData.SyncRelMove ? '.rel' : '.abs';
                        actualCmd = `${moveCmd} ${data.axis} = ${data.distance} go ${data.axis} wait sta[${data.axis}] & ${robot_cmd_1.RobotStatusMask.InMotion}`;
                    }
                }
                break;
